...
export function FriendsScreen({ user, onBack, T, onOpenProfile }) {
  ...
  const renderIdentity = (person, fallbackId) => (
                ...
                {friends.length === 0 ? (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 10, height: 10, borderRadius: '50%', background: f.online ? '#4CAF50' : '#555' }} />
                      <div>
                        {renderIdentity(f, f.id)}
                        <div style={{ color: '#666', fontSize: 11, marginTop: 2 }}>
                          {f.online ? T('onlineStatus') : T('offlineStatus')}
                        </div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => handleBlock(f.id)} style={btnSmall('rgba(255,87,34,.15)', '#FF5722')}>{T('blockUser')}</button>
                      <button onClick={() => handleRemove(f.id)} style={btnSmall('rgba(244,67,54,.15)', '#F44336')}>{T('removeFriend')}</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {tab === 'requests' && (
              <div>
                {requests.length === 0 ? (
                  ...
                  >
                    {renderIdentity({ displayName: r.displayName, username: r.username }, r.fromUserId)}
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => handleAccept(r.id)} style={btnSmall('rgba(76,175,80,.2)', '#4CAF50')}>{T('accept')}</button>
                      <button onClick={() => handleDecline(r.id)} style={btnSmall('rgba(244,67,54,.15)', '#F44336')}>{T('decline')}</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                  ...
                  <button onClick={handleSearch} disabled={searchLoading} style={{
                    ...
                    {searchLoading ? '...' : T('sendRequest').split(' ')[0]}
                  </button>
                </div>
